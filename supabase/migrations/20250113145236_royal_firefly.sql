...
-- Create payment method validation function
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND name = NEW.payment_method::text
  ) THEN
...
-- Ensure all users have the default payment methods
...
WHERE NOT EXISTS (
  ...
  AND pm.name = t.payment_method::text
);